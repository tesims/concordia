...
class EndToEndNegotiationTest(unittest.TestCase):
  ...
  def test_adaptive_strategy_negotiation(self):
    ...
    # Create adaptive GM (note: max_rounds already defaults to 20 in build_adaptive_negotiation)
    ...
    # Note: analyze_strategy_from_action doesn't add to _strategy_snapshots - we add manually
    self.assertEqual(len(evolution_module._strategy_snapshots), 0)
...
class SimulationRunnerTest(unittest.TestCase):
  ...
  def test_negotiation_state_lifecycle(self):
    ...
    # 4. Advance round (using actual API)
    state_component.advance_round('lifecycle_test')
    self.assertEqual(state.current_round, 1)

    # 5. Accept offer to complete negotiation (pass offer object and acceptor)
    state_component.accept_offer('lifecycle_test', offer2, 'Alice')
    self.assertTrue(offer2.is_accepted)
