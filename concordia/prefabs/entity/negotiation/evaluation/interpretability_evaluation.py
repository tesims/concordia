...
class InterpretabilityRunner:
    ...
    def save_dataset(self, filepath: str):
        ...
        all_agent_labels = []
        all_gm_labels = []
        all_outcome_labels = []  # NEW: Outcome-based labels
        counterpart_indices = []  # NEW: Cross-agent pairing
        metadata = []
        ...
        for sample in self.activation_samples:
            ...
            if layer_acts:
                ...
                metadata.append({
                    ...
                    # NEW: Multi-agent enhancement fields
        ...
        if all_activations:
            dataset = {
                ...
                # NEW: Cross-agent pairing indices (for alignment analysis)
...
if __name__ == "__main__":
    print("Interpretability + Evaluation Runner (v2 with GM Integration)")
    print("=" * 60)
    print("""
Usage:
    ...
    # Run with GM ground truth labels
    results = runner.run_study(
        scenario='fishery',
        agent_modules=['theory_of_mind'],
        gm_modules=['social_intelligence'],
    ...
    # - metadata: trial/round/agent info

Key distinction:
    - perceived_deception: "I think YOU are deceiving ME" (agent's belief)
    - actual_deception: "YOU are deceiving" (GM ground truth)

    For interpretability research, you typically want actual_deception
    as your target label for training probes.
""")
